...
class CtcCrossEntropyCriterion(FairseqCriterion):
    ...
    def normalize_frm_label(self, time_label, feat_len):
        tar = np.full(shape=(time_label.shape[0], feat_len), fill_value=self.task.dictionaries[1].pad_index,
                      dtype=np.short)
        label2feat_ratio = self.task.cfg.sample_rate / feat_len
    ...
    def forward(self, model, sample, reduce=True, **kwargs):
        ...
        melody_target = self.normalize_frm_label(sample["target_list"][1], y_melody.shape[2])
