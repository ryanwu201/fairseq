...
class BaseFairseqModel(nn.Module):
    ...
    def load_state_dict(
        ...
        for meter_name, meter in new_state_dict.items():
            if meter_name not in state_dict:
                logger.warning(f'The parameter {meter_name} is not found in the current model')
                continue
            if meter.shape != state_dict[meter_name].shape:
                logger.warning(
                    f'The parameter shape  {meter_name} is not matched:{state_dict[meter_name].shape} <==> {meter.shape}(checkpoint)')
                continue
            state_dict[meter_name] = meter
